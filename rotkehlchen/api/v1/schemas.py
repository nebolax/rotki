...
from rotkehlchen.assets.asset import AssetWithSymbol, AssetWithSymbolAndCryptoOracles, CryptoAsset, EvmToken, UnderlyingToken
...
class StakingQuerySchema(
    ...
    from_timestamp = TimestampField(load_default=Timestamp(0))
    to_timestamp = TimestampField(load_default=ts_now)
    asset = AssetField(expected_type=AssetWithSymbolAndCryptoOracles, load_default=None)
    ...
    def make_staking_query(  # pylint: disable=no-self-use
        ...
        asset_list: Optional[Tuple['AssetWithSymbolAndCryptoOracles', ...]] = None
...
class ModifiableSettingsSchema(Schema):
    ...
    main_currency = AssetField(expected_type=AssetWithSymbolAndCryptoOracles, load_default=None)
    # TODO: Add some validation to this field
...
class ExchangeRatesSchema(AsyncQueryArgumentSchema):
    currencies = DelimitedOrNormalList(MaybeAssetField(expected_type=AssetWithSymbolAndCryptoOracles), required=True)
...
class SingleAssetWithOraclesIdentifierSchema(Schema):
    asset = AssetField(required=True, expected_type=AssetWithSymbolAndCryptoOracles, form_with_incomplete_data=True)
...
class BalanceSnapshotSchema(Schema):
    ...
    asset_identifier = AssetField(required=True, expected_type=Asset, form_with_incomplete_data=True)
    amount = AmountField(required=True)
    usd_value = AmountField(required=True)
