...
from rotkehlchen.assets.asset import Asset, AssetWithSymbol, AssetWithSymbolAndCryptoOracles
...
class Trade(AccountingEventMixin):
    ...
    def __str__(self) -> str:
        return (
            ...
            f'of type {str(self.trade_type)} with base asset: {self.base_asset.resolve_to_asset_with_name().name} '
            f'and quote asset: {self.quote_asset.resolve_to_asset_with_name().name}'
        )
...
class MarginPosition(AccountingEventMixin):
    ...
    pl_currency: AssetWithSymbolAndCryptoOracles
    # Amount of fees paid
    fee: Fee
    # The asset in which fees were paid
    fee_currency: AssetWithSymbolAndCryptoOracles
...
class Loan(AccountingEventMixin):
    ...
    open_time: Timestamp
    close_time: Timestamp
    currency: AssetWithSymbolAndCryptoOracles
    fee: Fee
    earned: AssetAmount
...
def trade_pair_from_assets(base: AssetWithSymbolAndCryptoOracles, quote: AssetWithSymbolAndCryptoOracles) -> TradePair:
...
class BinancePair(NamedTuple):
    ...
    result"""
    symbol: str
    base_asset: AssetWithSymbolAndCryptoOracles
    quote_asset: AssetWithSymbolAndCryptoOracles
