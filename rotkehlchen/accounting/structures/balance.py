...
@dataclass(init=True, repr=True, eq=True, order=False, unsafe_hash=False, frozen=False)
class BalanceSheet:
    # TODO: looks like BalanceSheet is used only for CryptoAssets, but will have to check again
    assets: DefaultDict['CryptoAsset', Balance] = field(default_factory=lambda: defaultdict(Balance))
    liabilities: DefaultDict['CryptoAsset', Balance] = field(default_factory=lambda: defaultdict(Balance))  # noqa: E501
