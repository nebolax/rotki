...
class GlobalDBHandler():
    ...
    def delete_evm_token(
        ...
        GlobalDBHandler().delete_asset_by_identifier(write_cursor, asset_identifier)
    ...
    def add_user_owned_assets(assets: List['Asset']) -> None:
        ...
        try:
            ...
            log.error(
                f'One of the following asset ids doesnt exist: '
                f'{",".join([x.identifier for x in assets])}. {str(e)}',
            )  # should not ever happen but need to handle with informative log if it does

    @staticmethod
    def delete_asset_by_identifier(write_cursor: 'DBCursor', identifier: str) -> None:
