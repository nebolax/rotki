...
Getting Convex historical data
==============================

.. http:get:: /api/(version)/blockchains/ETH/modules/convex/history

   Doing a GET on the conex events history resource will return the history of staking events (e.g. withdraw, deposit, reward).
   ...
   **Example Request**:
   ...
   .. http:example:: curl wget httpie python-requests
      ...
      GET /api/1/blockchains/ETH/modules/convex/history HTTP/1.1
      Host: localhost:5042
   ...
   **Example Response**:
            ...
            "0xE74ad5437C6CFB0cCD6bADda1F6b57b6E542E75e": [
                {
                    "timestamp": 1604366004,
                    "tx_hash": "0x9989f47c6c0a761f98f910ac24e2438d858be96c12124a13be4bb4b3150c55ea",
                    "asset": "_ceth_0xADE00C28244d5CE17D72E40330B1c318cD12B7c3",
                    "event_type": "deposit",
                    "balance": {
                        "amount": "1000",
                        "usd_value": "950"
   ...
   :resjson object result: A mapping between accounts and their Convex events history
   :resjson list[object] events: A list of all the staking events generated by the address interacting with the pools.

       - tx_hash: The transaction hash of the event.
       - timestamp: The Unix timestamp in UTC when the event happened (in seconds).
       - event_type: The type of event. Can be: ``"deposit"``, ``"withdraw"`` and ``"reward"``.
       - asset: identifier of the asset that was used for the interaction.
       - balance: asset amount and usd value

   :statuscode 200: Convex events history successfully queried.
   :statuscode 409: User is not logged in.
   :statuscode 500: Internal rotki error.
   :statuscode 502: An external service used in the query such as etherscan or the graph node could not be reached or returned unexpected response.

Getting Convex balances
==============================

.. http:get:: /api/(version)/blockchains/ETH/modules/convex/balances

   Doing a GET on the Convex balances resource will return the amounts of tokens staked in the Convex pools.
   ...
   **Example Request**:
   ...
   .. http:example:: curl wget httpie python-requests
      ...
      GET /api/1/blockchains/ETH/modules/convex/balances HTTP/1.1
      Host: localhost:5042
   ...
   **Example Response**:
   ...
   :resjson object result: A mapping between accounts and their balances in the Convex pools (represented by a dictionary of asset to this asset's balance).

   :statuscode 200: Convex balances successfully queried.
   :statuscode 409: User is not logged in.
   :statuscode 500: Internal rotki error.
   :statuscode 502: An external service used in the query such as etherscan or the graph node could not be reached or returned unexpected response.